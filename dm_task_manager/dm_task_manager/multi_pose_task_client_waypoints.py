...
# class AdvancedCollisionAvoidancePlanner:
#     """고급 충돌 회피 계획기 - MuJoCo 연동 없이 독립적으로 동작"""
...
#     def is_collision_free(self, point, time_offset=0):
#         """점이 장애물과 충돌하지 않는지 확인 (동적 장애물 고려)"""
#         for obs in self.obstacles:
#             # 동적 장애물의 경우 시간에 따른 위치 예측
#             if hasattr(obs, 'is_dynamic') and obs.is_dynamic:
#                 predicted_pos = obs.position + obs.velocity * time_offset
...
#             dist = np.linalg.norm(point - predicted_pos)
            
#             if obs.shape == "sphere":
#                 # 구체의 경우: size는 반지름(float)
#                 safe_distance = obs.size + self.safety_margin
#                 if dist < safe_distance:
#                     return False, f"Too close to {obs.name} (dist: {dist:.3f}m, safe: {safe_distance:.3f}m)"
#             elif obs.shape == "box":
#                 # 박스의 경우: size는 [x,y,z] 리스트
#                 diff = np.abs(point - predicted_pos)
#                 box_half = np.array(obs.size) + self.safety_margin
#                 if all(diff[i] < box_half[i] for i in range(3)):
#                     return False, f"Inside {obs.name} box region"
                    
#         return True, "Safe"
    
#     def is_within_workspace(self, point):
#         """점이 로봇 작업공간 내에 있는지 확인"""
#         return (self.workspace_limits['x'][0] <= point[0] <= self.workspace_limits['x'][1] and
#                 self.workspace_limits['y'][0] <= point[1] <= self.workspace_limits['y'][1] and
#                 self.workspace_limits['z'][0] <= point[2] <= self.workspace_limits['z'][1])
    
#     def check_path_safety(self, start_pos, end_pos, num_checks=15):
#         """경로의 안전성을 세밀하게 검사"""
#         collision_points = []
#         unsafe_segments = []
...
#                 unsafe_segments.append((i, "Outside workspace"))
#                 continue
                
#             # 충돌 확인
#             safe, reason = self.is_collision_free(point, time_offset)
#             if not safe:
...
#     def plan_collision_free_path(self, start_pos, end_pos):
#         """충돌 없는 경로 계획 (디버깅 정보 추가)"""
        
#         print(f"🔍 DEBUG: Planning path from {start_pos} to {end_pos}")
#         print(f"🔍 DEBUG: Distance: {np.linalg.norm(end_pos - start_pos):.3f}m")
        
#         # 1단계: 직선 경로 검사
#         direct_safety = self.check_path_safety(start_pos, end_pos)
#         print(f"🔍 DEBUG: Direct path safety: {direct_safety['safe']}, score: {direct_safety['safety_score']:.2f}")
#         print(f"🔍 DEBUG: Unsafe segments: {len(direct_safety['unsafe_segments'])}")
        
#         if direct_safety['safe']:
#             return {
#                 'success': True,
#                 'waypoints': self.generate_linear_waypoints(start_pos, end_pos, 3),
#                 'strategy': 'direct_path',
#                 'safety_score': direct_safety['safety_score']
#             }
...
#                 waypoints = strategy_func(start_pos, end_pos)
#                 if waypoints and len(waypoints) >= 2:
#                     # 전체 경로의 안전성 평가
#                     total_score = self.evaluate_path_safety(waypoints)
#                     print(f"🔍 DEBUG: {strategy_name} safety score: {total_score:.2f}")
                    
#                     if total_score > 0.8:  # 80% 이상 안전하면 사용
#                         print(f"🔍 DEBUG: {strategy_name} accepted (score > 0.8)")
#                         return {
#                             'success': True,
#                             'waypoints': waypoints,
#                             'strategy': strategy_name,
...
#                         }
#                     elif total_score > best_score:
#                         print(f"🔍 DEBUG: {strategy_name} is new best (score: {total_score:.2f})")
#                         best_result = {
#                             'success': True,
#                             'waypoints': waypoints,
#                             'strategy': strategy_name,
...
#         # 최선의 결과 반환 (완전히 안전하지 않더라도)
#         if best_result and best_score > 0.3:  # 기준을 0.5에서 0.3으로 낮춤
#             print(f"🔍 DEBUG: Using best result: {best_result['strategy']} (score: {best_score:.2f})")
#             return best_result
        
#         print(f"🔍 DEBUG: All strategies failed. Best score: {best_score:.2f}")
#         return {
#             'success': False,
#             'waypoints': None,
#             'strategy': 'none',
...
#     def generate_safe_waypoints(self, start_pos, end_pos, num_intermediate=3):
#         """안전한 중간 웨이포인트 생성 (기본 버전)"""
#         waypoints = [start_pos]
        
#         # 단순한 중간점들 생성
#         for i in range(1, num_intermediate + 1):
#             alpha = i / (num_intermediate + 1)
#             intermediate = start_pos + alpha * (end_pos - start_pos)
...
#         # 하위 호환성을 위한 기본 계획기
#         self.trajectory_planner = WaypointTrajectoryPlanner(safety_margin=0.08)
...
#         self.get_logger().info("🤖 Advanced Collision-Aware Waypoint Client initialized!")
    
#     def setup_obstacles(self):
#         """시뮬레이션의 장애물 정보 수집 및 설정 (임시로 비활성화)"""
#         self.get_logger().info("🚧 Setting up obstacle information (DEBUG MODE)...")
...
#         for obs in static_obstacles:
#             self.collision_planner.add_obstacle(obs)
#             self.trajectory_planner.add_obstacle(obs)  # 하위 호환성
            
#         # 동적 장애물도 비활성화
#         # moving_velocity = np.array([0.02, 0.01, 0.0])
#         # self.collision_planner.add_dynamic_obstacle(
#         #     "moving_obstacle", [0.6, 0.0, 0.6], 0.06, moving_velocity
#         # )
        
#         self.get_logger().info(f"   Added {len(static_obstacles)} static obstacles (DEBUG: reduced)")
#         self.get_logger().info("   Added 0 dynamic obstacles (DEBUG: disabled)")
#         self.get_logger().info("🛡️ DEBUG: Simplified collision avoidance for testing!")
...
#     def execute_waypoint_sequence(self):
#         """웨이포인트 시퀀스 실행 (고급 충돌 회피 포함)"""
#         self.get_logger().info("🎯 Starting advanced waypoint sequence with collision avoidance...")
...
#         # 이동 시간 설정
#         duration = 5.0  # 각 웨이포인트 간 5초
#         wait_time = 1.5  # 웨이포인트 도달 후 대기 시간 (약간 늘림)
...
#             # 🛡️ 고급 충돌 회피 경로 계획
#             self.get_logger().info("🛡️ Planning collision-free path with advanced algorithms...")
            
#             path_result = self.collision_planner.plan_collision_free_path(ee_pos, target_pos)
#             self.path_stats['total_paths'] += 1
            
#             if not path_result['success']:
#                 self.get_logger().error(f"❌ Could not find safe path to {waypoint_name}")
#                 self.path_stats['failed_paths'] += 1
#                 continue
...
#             safe_waypoints = path_result['waypoints']
#             strategy = path_result['strategy']
#             safety_score = path_result['safety_score']
            
#             self.get_logger().info(f"✅ Path planned successfully:")
#             self.get_logger().info(f"   Strategy: {strategy}")
#             self.get_logger().info(f"   Safety score: {safety_score:.2f}")
#             self.get_logger().info(f"   Waypoints: {len(safe_waypoints)}")
            
#             # 생성된 웨이포인트들을 순차적으로 이동
#             for j, waypoint_pos in enumerate(safe_waypoints):
#                 if j == 0:  # 첫 번째는 현재 위치이므로 스킵
#                     continue
                    
#                 # EE 프레임 기준 상대 위치로 변환
#                 rel_pos = R_ee.T.dot(waypoint_pos - ee_pos)
                
#                 # 상대 위치가 너무 크면 제한 (더 보수적)
#                 max_step = 0.1  # 10cm 제한
#                 if np.linalg.norm(rel_pos) > max_step:
#                     rel_pos = rel_pos / np.linalg.norm(rel_pos) * max_step
#                     self.get_logger().warn(f"   Step size limited to {max_step}m")
                
#                 # 포즈 생성 및 전송
#                 target_pose = pose_from_xyz_quat(rel_pos, welding_quat)
                
#                 self.get_logger().info(f"   🚀 Safe waypoint {j}/{len(safe_waypoints)-1}: {rel_pos} (strategy: {strategy})")
                
#                 success = self.send_pose(target_pose, duration)
#                 if not success:
#                     self.get_logger().error(f"❌ Failed to reach safe waypoint {j}")
#                     return False
...
#         if overall_success:
#             self.get_logger().info(f"🎉 Trajectory completed! ({success_count}/{self.path_stats['total_paths']} waypoints reached)")
#         else:
#             self.get_logger().error("❌ All waypoints failed!")
...
#     def print_path_statistics(self):
#         """경로 계획 통계 출력"""
#         self.get_logger().info("📊 Path Planning Statistics:")
#         self.get_logger().info(f"   Total paths: {self.path_stats['total_paths']}")
#         self.get_logger().info(f"   Direct paths: {self.path_stats['direct_paths']}")
#         self.get_logger().info(f"   Avoidance paths: {self.path_stats['avoidance_paths']}")
#         self.get_logger().info(f"   Failed paths: {self.path_stats['failed_paths']}")
...
#         if success:
#             node.get_logger().info("🎉 Advanced welding trajectory completed successfully!")
#         else:
#             node.get_logger().error("❌ Welding trajectory failed!")
...
# #     def setup_obstacles(self):
# #         """시뮬레이션의 장애물 정보 수집 및 설정"""
# #         self.get_logger().info("🚧 Setting up advanced obstacle information...")
        
# #         # 정적 장애물들
# #         static_obstacles = [
# #             ObstacleInfo("static_clamp1", [0.38, 0.08, 0.49], [0.02, 0.02, 0.05], "box"),
# #             ObstacleInfo("tool_obstacle", [0.65, -0.1, 0.49], [0.03, 0.03, 0.08], "box"),
# #             ObstacleInfo("worker_torso", [0.5, -0.25, 0.55], 0.12, "sphere"),
# #             ObstacleInfo("curved_pipe", [0.45, 0.2, 0.49], [0.025, 0.025, 0.15], "box"),
# #         ]
...
# #         # 동적 장애물 (움직이는 작업자 손)
# #         moving_velocity = np.array([0.02, 0.01, 0.0])  # 2cm/s로 움직임
# #         self.collision_planner.add_dynamic_obstacle(
# #             "moving_obstacle", [0.6, 0.0, 0.6], 0.06, moving_velocity
# #         )
        
# #         self.get_logger().info(f"   Added {len(static_obstacles)} static obstacles")
# #         self.get_logger().info("   Added 1 dynamic obstacle")
# #         self.get_logger().info("🛡️ Advanced collision avoidance system ready!")
...
# # # #             if obs.shape == "sphere":
# # # #                 if dist < (obs.size + self.safety_margin):
# # # #                     return False
# # # #             elif obs.shape == "box":
# # # #                 # 간단한 박스 충돌 검사 (중심 기준)
# # # #                 diff = np.abs(point - obs.position)
# # # #                 if all(diff[i] < (obs.size[i] + self.safety_margin) for i in range(3)):
# # # #                     return False
# # # #         return True
...
# # # #         # 직선 경로가 안전한지 확인
# # # #         if self.is_path_collision_free(start_pos, end_pos):
# # # #             # 직선 경로로 중간점들 생성
# # # #             for i in range(1, num_intermediate + 1):
# # # #                 alpha = i / (num_intermediate + 1)
# # # #                 intermediate = start_pos + alpha * (end_pos - start_pos)
...
# # # #         else:
# # # #             # 장애물을 피하는 경로 생성
# # # #             waypoints.extend(self.generate_detour_waypoints(start_pos, end_pos, num_intermediate))
        
# # # #         waypoints.append(end_pos)
# # # #         return waypoints
    
# # # #     def is_path_collision_free(self, start, end, num_checks=20):
# # # #         """경로가 충돌 없는지 확인"""
# # # #         for i in range(num_checks + 1):
# # # #             alpha = i / num_checks
...
# # # #     def generate_detour_waypoints(self, start_pos, end_pos, num_points):
# # # #         """우회 경로 생성"""
# # # #         waypoints = []
        
# # # #         # 장애물을 피하기 위해 위쪽으로 우회하는 경로 생성
# # # #         mid_point = (start_pos + end_pos) / 2
        
# # # #         # Z축으로 올라가서 우회
# # # #         detour_height = 0.15  # 15cm 위로
# # # #         elevated_point = mid_point + np.array([0, 0, detour_height])
        
# # # #         # 안전한 중간점들 생성
# # # #         for i in range(1, num_points + 1):
# # # #             if i <= num_points // 2:
# # # #                 # 올라가는 구간
# # # #                 alpha = (2 * i) / num_points
# # # #                 point = start_pos + alpha * (elevated_point - start_pos)
# # # #             else:
# # # #                 # 내려가는 구간
# # # #                 alpha = (2 * (i - num_points // 2)) / num_points
# # # #                 point = elevated_point + alpha * (end_pos - elevated_point)
...
# # # #         self.get_logger().info("🤖 Waypoint-based Welding Client initialized!")
    
# # # #     def setup_obstacles(self):
# # # #         """시뮬레이션의 장애물 정보 수집"""
# # # #         self.get_logger().info("🚧 Setting up obstacle information...")
        
# # # #         # 정적 장애물들의 대략적인 위치 (welding_scene2.xml 기준)
# # # #         obstacles = [
# # # #             # 움직이는 장애물 (작업자 손)
# # # #             ObstacleInfo("moving_obstacle", [0.6, 0.0, 0.6], 0.05, "sphere"),
...
# # # #             self.trajectory_planner.add_obstacle(obs)
# # # #             self.get_logger().info(f"   Added obstacle: {obs.name}")
...
# # # #     def execute_waypoint_sequence(self):
# # # #         """웨이포인트 시퀀스 실행"""
# # # #         self.get_logger().info("🎯 Starting waypoint sequence execution...")
...
# # # #         # 이동 시간 설정 (더 길게)
# # # #         duration = 5.0  # 각 웨이포인트 간 5초로 증가
# # # #         wait_time = 1.0  # 웨이포인트 도달 후 대기 시간
...
# # # #             # 안전한 경로 생성
# # # #             safe_waypoints = self.trajectory_planner.generate_safe_waypoints(
# # # #                 ee_pos, target_pos, num_intermediate=1  # 중간점 줄임
# # # #             )
            
# # # #             self.get_logger().info(f"   Generated {len(safe_waypoints)} waypoints for safe trajectory")
            
# # # #             # 생성된 웨이포인트들을 순차적으로 이동
# # # #             for j, waypoint_pos in enumerate(safe_waypoints[1:], 1):  # 첫 번째는 현재 위치
# # # #                 # EE 프레임 기준 상대 위치로 변환
# # # #                 rel_pos = R_ee.T.dot(waypoint_pos - ee_pos)
                
# # # #                 # 상대 위치가 너무 크면 제한 (더 보수적)
# # # #                 max_step = 0.12  # 12cm 제한으로 줄임
# # # #                 if np.linalg.norm(rel_pos) > max_step:
# # # #                     rel_pos = rel_pos / np.linalg.norm(rel_pos) * max_step
# # # #                     self.get_logger().warn(f"   Step size limited to {max_step}m")
...
# # # #                 self.get_logger().info(f"   Sub-waypoint {j}/{len(safe_waypoints)-1}: rel_pos={rel_pos}")
                
# # # #                 success = self.send_pose(target_pose, duration)
# # # #                 if not success:
# # # #                     self.get_logger().error(f"❌ Failed to reach sub-waypoint {j}")
# # # #                     return False
...
# # # #         if success:
# # # #             node.get_logger().info("🎉 Welding trajectory completed successfully!")
# # # #         else:
# # # #             node.get_logger().error("❌ Welding trajectory failed!")
...
# # # # #             # 안전한 경로 생성
# # # # #             safe_waypoints = self.trajectory_planner.generate_safe_waypoints(
# # # # #                 ee_pos, target_pos, num_intermediate=2
# # # # #             )
...
# # # # #                 self.get_logger().info(f"   Sub-waypoint {j}/{len(safe_waypoints)-1}: {rel_pos}")
